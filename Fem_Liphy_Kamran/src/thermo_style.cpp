...
{
	cSxyObj->Init( elem->stran, elem->strst, elem->vol, 
...
//				cout << femObj->get_potnt() << '\t';
		...
		if ( !strcmp( args[ iarg ], "sxy" ) )
		{ 
			fprintf( output, "%15.14e\t", cSxyObj->GetSxy() );
		}
		if ( !strcmp( args[ iarg ], "p" ) )
		{ 
			fprintf( output, "%15.14e\t", cSxyObj->GetP() );
		}
		if ( !strcmp( args[ iarg ], "ev" ) )
		{ 
			fprintf( output, "%15.14e\t", cSxyObj->GetEv() );
		}
		if ( !strcmp( args[ iarg ], "n" ) )
		{ 
			kount = 0;
...
//				#pragma omp atomic
		...
		if ( !strcmp( args[ iarg ], "ke" ) )
		{ 
			fprintf( output, "%15.14g\t", cKEobj->GetKE() / node->nsvab );
		}
		if ( !strcmp( args[ iarg ], "lohi" ) )
			fprintf( output, "%g\t", itime * ts->dt * fd->shearRATE );
	}
	fprintf( output, "\n" );
}
